...
def process_file() -> None:
    ...
    print(f"Ищу input.txt в: {current_dir}")
    print(f"Полный путь: {input_file}")
    
    # 检查文件是否存在
    if not os.path.exists(input_file):
        print(f"[ERROR] Ошибка: Файл не найден: {input_file}")
        print("Файлы в директории task5:")
    ...
    try:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            try:
                ...
                parts = line.split()
                if len(parts) != 2:
                    print(f"[WARNING] Предупреждение: строка {line_num} содержит {len(parts)} чисел (нужно 2): '{line}'")
                    continue
                ...
                print(f"[ERROR] Ошибка в строке {line_num}: неверный формат числа в '{line}'")
...
def main():
    ...
    try:
        with open(output_file, 'r', encoding='utf-8') as f:
            print("\n[FILE] Содержимое файла output.txt:")
            print("-" * 30)
            print(f.read())
    except FileNotFoundError:
        print(f"\n[WARNING] Файл {output_file} не найден")
